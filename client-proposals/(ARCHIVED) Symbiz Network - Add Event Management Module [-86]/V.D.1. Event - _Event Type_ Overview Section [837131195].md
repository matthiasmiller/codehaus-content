5.4.1. Event - <Event Type> Overview Section
...
"<Event Type> Overview" section (visible on all Event Types; visibility for users based on "Event Status" and the "Event Visibility" field selection): 
  ...
  * Event Type (required; drop list of Event Type list items; defaults to blank or as set by the creation link; when an option is selected (either initially or changed), the Event Visibility is immediately updated to match the Type's "Default Event Visibility", if one is set; warning on the field if changed after the initial save:  "Changing the Type for an existing Event may cause fields to be blanked out when the record is saved."; when out of edit mode, this field is not a link)
  * Event Name (required; plain text field)
  * Event Focus (optional; plain text field) 
  * Event Visibility (required; field visible to all contacts who have a Registration record for the Event and to all Regional Reps and Super Admins; drop list of Event Visibility list items - see corresponding section of the proposal; defaults to the selection on the Event Type record for the selected Type; this controls the visibility of all non-Regional Rep / Super Admin fields on the record, as follows: 
...
Tim Reitz 05/13/2024: For the Time Zone field, see [[[IN9889](https://zch.apphosting.zone/Detail/Edit/2?RecordType=Incidents&NumberID=-9891&)]] - ZSB - Add Time Zone Feature (for Groups & GR Meetings).

  


Tim Reitz 05/17/2024: Note that we have 3 similar items here: 
...
Let's make sure we keep them separate and put the right items in the right places :) If something doesn't seem right in the spec, please reach out so we can clarify. Thanks!
...
[ ] Event Status can be modeled off of Growth Ring Group Status. 2 hour

[ ] Event ID is the lookup key from fields. with a bit of validation for duplicates (don't validate against yourself).  2 hours

[ ] Event Scheduled/Cancel/Complete are cloned from Growth Ring Group records. 2 hours

  


[ ] Time Zone coded in [[[IN9889](https://zch.apphosting.zone/Detail/Edit/2?RecordType=Incidents&NumberID=-9891&)]]

[ ] Deadline to Register -- Required: Status="Scheduled" OR Type="Launch Meeting"

[ ] View My Registration 1 hour

[ ] Other fields and links 3 hours
