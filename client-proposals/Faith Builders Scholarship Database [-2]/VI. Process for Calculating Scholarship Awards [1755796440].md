6\. Process for Calculating Scholarship Awards
...
If the school opts for discretionary funds, the State requires that 80% of the funds must go to tuition. FB enforces a limit of up to 13% of the gross funds (before the management fee) being allocated to discretionary funds.
...
For each of the fund accounts, we will provide the option to choose between a percentage or dollar value for the discretionary fund. This must not exceed 13% of the gross amount to be distributed (<= Distribution * .13 / .82). We will default these values based on the school's application (either 0% or 13%).
