8.8.6. Traccar Event Sync Record
...
Sections and Fields: 
  ...
  * Traccar Event Details section (all fields in this section are set from Traccar and read-only in Silverloom): 
    * Traccar Event ID (auto-set and read-only interactively; number; no decimals; data synced from Traccar; corresponds to the "ID" in Traccar)
    * Event Type (auto-set and read-only interactively; plain text field; data synced from Traccar; corresponds to the "Type" in Traccar)
    * Traccar Event Time (auto-set and read-only interactively; UTC date & time in the ISO 8601 format (i.e. "2019-08-24T14:15:22Z"); data synced from Traccar; corresponds to the "Event Time" in Traccar)
    * Event Time Zone (auto-set and read-only; __; data synced from Traccar;__)
...
TODO_JB (research): Tim Reitz 10/25/2025: I think we'd have to look at the Event location (Position ID?) to determine this. 
  ...
  * Traccar Device ID (auto-set and read-only interactively; number; no decimals; data synced from Traccar; corresponds to the "Device ID" in Traccar)
  * Traccar Position ID (auto-set and read-only interactively; number; no decimal; data synced from Traccars; corresponds to the "Position ID" in Traccar; link to the corresponding Traccar Position Record in Silverloom) 
  * Traccar Coordinates (auto-set and read-only interactively; number; no decimals; data synced from Traccar; corresponds to the __ in Traccar) 
...
TODO_JB (research): Tim Reitz 01/27/2026: Is there something like this on the Event, or is it derived from a linked Position? (I'm seeing an option in the left-hand column of Events report in Traccar: ![](data:image/png;base64,) (see [https://x6rildfvs.traccar.com/reports/event](https://x6rildfvs.traccar.com/reports/event)) 
  ...
  * Traccar Geofence ID (auto-set and read-only interactively; number; no decimals; data synced from Traccar; corresponds to the "Geofence ID" in Traccar; link to the corresponding Traccar Geofence Record in Silverloom; note that this is not in use for Phase 1)
  * Traccar Logged VIN (auto-set and read-only interactively; plain text field; sets to the VIN of the vehicle from which the Event was logged; note that this VIN is used to create new Vehicle records in Silverloom and for other linking & reporting) 



TODO_VA (later): Tim Reitz 02/04/2026: data synced from Traccar? Or from linked Device? 
...
TODO_VA (later): Tim Reitz 01/27/2026: Spec once we have info on the User attributes from JB.
  ...
  * Silverloom Details section (some fields in this section are editable interactively, allowing Upline Providers to make "override" notes) : 
...
TODO_VA (later): Tim Reitz 01/31/2026: Spec this out once we have an answer.

Note that this impacts other places where Events are displayed (reports, the Events virtual RG on the Device)
...
Tim Reitz 01/31/2026: I'm wondering if this is the actual posted speed limit for the road, or if it's a limit that can be set on the driver / device...
