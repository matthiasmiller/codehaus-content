8.2.1. Contact Record - Contact Section
...
*. 
  ...
  * Short Display Name (standard; hidden; read-only macro; with the following details / behaviors: 
    * dynamically displays the following:
      * if "Is Organization" checkbox is checked: displays the following: "<Organization Name>";
      * if "Is Organization" checkbox is not checked: displays the following: "<First Name> <Last Name>";
    * hidden by default on the Contact, since it normally would not be needed here; to be used in places such as printouts and emails, where the address should not be included in the displayed name; 
    * custom behavior: on the first Save after this is updated, the "Update Short Display Name References" triggered automatic process - see corresponding spec) 
