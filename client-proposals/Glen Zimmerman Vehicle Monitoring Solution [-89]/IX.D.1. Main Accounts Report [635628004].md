...
TODO_VA: Tim Reitz 02/02/2026: Any on the administrators menu? 
...
Title: "Accounts", with the following prefixes and/or suffixes: 
  ...
  * if "Account Reseller" is not blank: includes " (Reseller: <Short Display Name>)" suffix
  * if "Account Manager" is not blank: includes " (Account Manager: <Display Name>)" suffix
  * if "Primary Only" is checked and "Account Manager" is not blank: includes " (Primary Account Manager: <Display Name>)" suffix
...
TODO_VA: plain text or drop list?
  ...
  * Account Manager (optional; drop list; with the following details / behaviors: 
    * if "Primary Only" = not checked: drop list includes all Account Managers; 
    * if "Primary Only" = checked: drop list includes only Primary Account Managers; 
    * defaults to blank = all; 
    * when one is selected, the report displays all Accounts for which that Contact is included on the "Account Managers" embedded spreadsheet)
  * Primary Only (checkbox; defaults to not checked; when checked, the "Account Manager" filter includes only Primary Account Managers) 
