8.8.3. Traccar Group Sync Record
...
Jonathan Bergen 02/02/2026: I think it would make sense to have it be a Silverloom record <todo... supper time>
...
Sections and Fields:
  ...
  * Traccar Group Details section: 
    * Unique ID (auto-set and read-only interactively; number; no decimals; data synced with Traccar; corresponds to the unique ID from the Traccar Group; note that this is the ID included in the URL in Traccar)
    * Name (auto-set and read-only interactively; plain text field; data synced with Traccar; corresponds to the "Name" in Traccar)
    * Attributes (auto-set and read-only interactively; __ field; ; data synced with Traccarcorresponds to the __ in Traccar)



TODO_VA (later): Tim Reitz 01/27/2026: Spec once we have info on the User attributes from JB.

  * Upline Group (auto-set and read-only interactively; list field of Traccar Group records; data synced with Traccar; corresponds to the "Group" in Traccar) 
