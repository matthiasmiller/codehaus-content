1.5.2.2. Schedule and Progress Section
...
New / updated spec for existing Schedule and Progress section (blue font represents changes to existing design / implementation):
  ...
  * Right column: 
    ...
    * Calculated Project Finish Date (note: previously called Projected Finish Date) (auto-set and read-only; date field with the following behaviors:
      * for non-child incidents:
        * visible if Scheduled checkbox is checked;
...
[ ] Estimated Hours -- minor changes in behavior and validation related to linking
...
[ ] Machine on Site -- Stored + Macro + validation
...
[ ] QC Passed Checkbox
  ...
  * There is a stored hidden Yes/No QC Passed field on all incidents.
  * There is a displayed editable expression/macro "QC Passed" checkbox on all incidents
    * For child incidents, it is checked if the child's hidden field is set to Yes or if the parent's hidden field is set to Yes
    * For parent incidents:
      * it is checked if:
        * if the parent's hidden field is set to Yes (and no child is set to No), or
        * if the parent's hidden field is blank and all child incidents' hidden fields are set to Yes
      * otherwise it is unchecked



For non-parent, non-child incidents: it is checked if the hidden field is set to Yes

Tim Reitz 11/15/2023: Visual explanation from conversation with Ellis today: [https://docs.google.com/spreadsheets/d/16AFeBdYOBsvB3i6AygKdMdmfbxH6Jw3pbB_M44KW5cA/](https://docs.google.com/spreadsheets/d/16AFeBdYOBsvB3i6AygKdMdmfbxH6Jw3pbB_M44KW5cA/) 
...
[ ] Finished becomes a Stored Field + Macro
...
[ ] Service Runs: Macro that counts these. Probably use our TimeCardByIncidentID(?) ndx for parent and all children and concat "DateSortString(TImeSlotDate)+SvcRunNumber" for all items with SvcRun numbers, then SortPipeList and CountPipeList. If two people have the same SvcRun for a vehicle for a day, we'll call it the same Svc Run.

[ ] We want to freeze this number when Billed is checked. Add a StoredSvcRuns macro (used only on parent record). When Billed is checked, set StoredSvcRuns = CalcSvcRuns.

[ ] Be sure to set to zero if there were no service rusn.

[ ] On Uncheck, blank out stored number.

[ ] A SvcRuns macro will return ReplaceNA(StoredSvcRuns, CalcSvcRuns) for non-child incidents. For child, we look it up.
...
[ ] Changes to Billed -- stored and macro and validation
...
[ ] Scheduled checkbox -- Stored and Macro
...
[ ] Actual Hours -- NOTE: Will need to store an RG of all project incidents + Stored Hours when Billed is checked.
...
[ ] Travel Time -- Stored and Macro

[ ] Checking "Override" makes the macro editable and does an OnChange into the the Stored field.
...
[ ] Target Start Date -- field with some validation
...
The various readonly fields in the last column are basically provided in our scheduling module separately.
